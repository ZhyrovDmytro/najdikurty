...
import {
  detectPadelSlaviaActiveDayMonth,
  parsePadelSlaviaAvailability
} from "../src/providers/padelslavia/parser.js";
...
describe("parsePadelSlaviaAvailability", () => {
  ...
  it("detects the active day shown above the timetable", () => {
    const html = readFileSync(new URL("./fixtures/padelslavia.html", import.meta.url), "utf8");

    expect(detectPadelSlaviaActiveDayMonth(html)).toBe("08-04");
  });
});
